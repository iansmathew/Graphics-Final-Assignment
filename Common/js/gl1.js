...
function GLInstance(canvasID) {
    ...
    //the purpose of mesh is to have a place that hold all the scene data so we can clear them call if we need to
    ...
    gl.fCreateMeshVAO = function (name, aryInd, aryVert, aryNorm, aryUV, aryColor) {
        var rtn = { drawMode: this.TRIANGLES };

        //Create and bind vao in WebGL 2.0
        //rtn.vao = this.createVertexArray();
        //In Firefox, you can use this

        var ext = gl.getExtension('OES_vertex_array_object');
        rtn.vao = ext.createVertexArrayOES();
        ext.bindVertexArrayOES(rtn.vao);
        ...
        //.......................................................
        //Setup COLOR
        if (aryColor !== undefined && aryColor != null) {
            rtn.bufColor = this.createBuffer();
            this.bindBuffer(this.ARRAY_BUFFER, rtn.bufColor);
            this.bufferData(this.ARRAY_BUFFER, new Float32Array(aryColor), this.STATIC_DRAW);
            this.enableVertexAttribArray(ATTR_COLOR_LOC);
            this.vertexAttribPointer(ATTR_COLOR_LOC, 3, this.FLOAT, false, 0, 0);	//Color only has three floats per component
        }
        ...
        //Clean up
        ext.bindVertexArrayOES(null);					//Unbind the VAO, very Important. always unbind when your done using one.
